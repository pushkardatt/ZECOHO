...
-   **Room Type & Meal Plan Management**: Comprehensive room type and meal option system enabling OTA-style property configuration.
    ...
    - **Customer UI**: OTA-style room selection on property details page showing room types as cards with features, meal plan dropdown, and price breakdown. Price updates dynamically based on room type + meal option + nights + rooms. Uses shared helper `getMealOptionsForDisplay()` for consistent fallback handling when room types have no meal options.
