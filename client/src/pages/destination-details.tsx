...
export default function DestinationDetails() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>
          Hotels in {destination.name}, {destination.state} — Direct Booking |
          ZECOHO
        </title>
        <meta
          name="description"
          content={`Explore verified hotels in ${destination.name}. Book directly with zero commission. Save 15-25% on every stay. ${destination.shortDescription?.slice(0, 100) || ""}`}
        />
        <meta
          property="og:title"
          content={`Hotels in ${destination.name} — ZECOHO`}
        />
        <meta
          property="og:description"
          content={`Book hotels in ${destination.name}, ${destination.state} directly. Zero commission. Best prices guaranteed.`}
        />
        <link
          rel="canonical"
          href={`https://www.zecoho.com/destinations/${destination.id}`}
        />
      </Helmet>
      <div className="container px-4 md:px-6 py-8">
        <Button
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="flex items-start justify-between gap-4">
              ...
              {destination.isFeatured && (
                <Badge
                  variant="secondary"
                  className="bg-yellow-500/90 text-white border-0"
                >
...
function generateThingsToDo(destination: Destination): string[] {
  ...
  const thingsToDoMap: Record<string, string[]> = {
    jaipur: [
      ...
      "Shop for traditional textiles at Johari Bazaar",
      "Visit the astronomical observatory Jantar Mantar",
      "Experience a traditional Rajasthani dinner",
    ],
    udaipur: [
      "Take a boat ride on Lake Pichola",
      "Explore the grand City Palace",
      "Watch cultural performances at Bagore Ki Haveli",
      "Visit the peaceful Saheliyon Ki Bari garden",
      "Enjoy rooftop dining with lake views",
    ],
    goa: [
      "Relax on the pristine beaches of Palolem or Baga",
      "Explore Old Goa's Portuguese churches",
      "Try water sports at Calangute Beach",
      "Visit the vibrant Saturday Night Market",
    ...
    mumbai: [
      ...
      "Experience the bustling Colaba Causeway",
      "Tour the famous Bollywood studios",
    ],
    delhi: [
      "Explore the historic Red Fort",
      "Visit the towering Qutub Minar",
      "Shop at Chandni Chowk markets",
      "Pay respects at India Gate",
      "Experience the peaceful Lotus Temple",
    ],
    agra: [
      "Marvel at the Taj Mahal at sunrise",
      "Explore the massive Agra Fort",
      "Visit the abandoned city of Fatehpur Sikri",
      "Shop for marble inlay crafts",
      "Taste authentic Mughlai cuisine",
    ],
    varanasi: [
      "Attend the evening Ganga Aarti ceremony",
      "Take a sunrise boat ride on the Ganges",
      "Explore the narrow galis and ghats",
    ...
    manali: [
      ...
      "Experience paragliding adventures",
      "Explore the hot springs at Vashisht",
      "Take a trip to Rohtang Pass",
    ],
    shimla: [
      "Walk along the iconic Mall Road",
    ...
    darjeeling: [
      ...
      "Ride the UNESCO heritage toy train",
      "Explore the Peace Pagoda",
      "Visit the Himalayan Mountaineering Institute",
    ],
    rishikesh: [
      "Practice yoga at an ashram",
      "Go white water rafting",
      "Walk across Laxman Jhula",
      "Attend the evening aarti at Triveni Ghat",
      "Try bungee jumping or cliff diving",
    ],
    amritsar: [
      "Seek blessings at the Golden Temple",
      "Witness the Wagah Border ceremony",
      "Pay homage at Jallianwala Bagh",
      "Feast on Amritsari kulcha and lassi",
      "Explore the historic Partition Museum",
    ],
  };
...
function getIconForItem(item: string, index: number): JSX.Element {
  ...
  if (
    ...
    lowerItem.includes("cuisine") ||
    lowerItem.includes("restaurant")
  ) {
    return <UtensilsCrossed className="h-4 w-4 text-primary" />;
  }
  if (
    lowerItem.includes("mountain") ||
    lowerItem.includes("hill") ||
    lowerItem.includes("trek") ||
    lowerItem.includes("valley")
  ) {
  ...
  if (
    ...
    lowerItem.includes("bazaar") ||
    lowerItem.includes("handicraft")
  ) {
    return <ShoppingBag className="h-4 w-4 text-primary" />;
  }
  if (
    lowerItem.includes("temple") ||
    lowerItem.includes("fort") ||
    lowerItem.includes("palace") ||
    lowerItem.includes("heritage") ||
    lowerItem.includes("monument")
  ) {
    return <Landmark className="h-4 w-4 text-primary" />;
  }
  if (
    lowerItem.includes("view") ||
    lowerItem.includes("sunset") ||
    lowerItem.includes("sunrise") ||
    lowerItem.includes("scenic")
  ) {
