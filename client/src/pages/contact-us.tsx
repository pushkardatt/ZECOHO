...
export default function ContactUs() {
  ...
  const hasCustomerSupport =
    settings?.customerSupportEmail || settings?.customerSupportPhone;
  const hasOwnerSupportData =
    settings?.ownerSupportEmail || settings?.ownerSupportPhone;
  const showOwnerSupport = hasOwnerSupportData && isOwner;
  const hasGrievanceOfficer =
    settings?.grievanceOfficerName || settings?.grievanceOfficerEmail;
  const hasPrivacyContact =
    settings?.privacyEmail || settings?.dataProtectionOfficerName;
  const hasBusinessContact = settings?.businessEmail || settings?.businessPhone;
  const hasRegisteredOffice =
    settings?.registeredOfficeName || settings?.registeredOfficeAddress;

  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Contact ZECOHO — We're Here to Help</title>
        <meta
          name="description"
          content="Get in touch with ZECOHO support. We're here to help with bookings, property listings, and any questions about India's zero commission hotel booking platform."
        />
        <link rel="canonical" href="https://www.zecoho.com/contact" />
      </Helmet>
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {settings?.customerSupportHours && (
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      ...
                      data-testid="text-support-hours"
                    >
                      {settings.customerSupportHours}
                    </span>
                  </div>
          ...
          {hasGrievanceOfficer && (
            <Card
              ...
              data-testid="card-grievance-officer"
            >
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                    ...
                    {settings?.grievanceOfficerName && (
                      <div>
                        <span
                          ...
                          data-testid="text-grievance-name"
                        >
                          {settings.grievanceOfficerName}
                        </span>
                    ...
                    {settings?.grievanceOfficerAddress && (
                      <div className="flex items-start gap-3">
                        ...
                        <span
                          ...
                          data-testid="text-grievance-address"
                        >
                          {settings.grievanceOfficerAddress}
                        </span>
                      </div>
                ...
                <div className="bg-muted/50 rounded-md p-4">
                  <p className="text-sm text-muted-foreground">
                    ...
                    <strong>48 hours</strong> and resolved within{" "}
                    <strong>1 month</strong> from the date of receipt, in
                    accordance with the Information Technology (Intermediary
          ...
          {hasRegisteredOffice && (
            <Card
              className={
                !hasPrivacyContact && !hasBusinessContact ? "md:col-span-2" : ""
              }
              data-testid="card-registered-office"
            >
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                ...
                {settings?.companyRegistrationNumber && (
                  <>
                    ...
                    <div className="text-sm">
                      ...
                      <span className="font-mono" data-testid="text-cin">
                        {settings.companyRegistrationNumber}
                      </span>
                    </div>
                  </>
