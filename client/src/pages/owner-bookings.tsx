...
export default function OwnerBookings() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return tabParam &&
      ["all", "pending", "upcoming", "ongoing", "past", "cancelled"].includes(
        tabParam,
      )
      ? tabParam
      : "pending";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const el = document.querySelector(
        `[data-testid="booking-card-${highlightBookingId}"]`,
      );
      if (el) {
  ...
  useEffect(() => {
    const handleSwMessage = (event: MessageEvent) => {
      if (event.data?.type === "BOOKING_ACTION") {
        ...
        if (action === "accept") {
          apiRequest("PATCH", `/api/owner/bookings/${bookingId}/status`, {
            status: "confirmed",
          })
            .then(() => {
  ...
  const [earlyCheckoutBooking, setEarlyCheckoutBooking] =
    useState<Booking | null>(null);
  const [extendStayDialogOpen, setExtendStayDialogOpen] = useState(false);
  const [extendStayBooking, setExtendStayBooking] = useState<Booking | null>(
    null,
  );
  ...
  const REJECTION_REASONS = [
    {
      ...
      label: "Property is not available for the requested dates",
    },
    { value: "fully_booked", label: "We are fully booked during this period" },
    { value: "maintenance", label: "Property is under maintenance" },
    {
      value: "minimum_stay",
      label: "Booking doesn't meet our minimum stay requirements",
    },
    {
      value: "guest_count",
      label: "The number of guests exceeds our capacity",
    },
    {
      value: "policy_violation",
      label: "Request doesn't comply with our property policies",
    },
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest("PATCH", `/api/owner/bookings/${id}/status`, {
        status,
        responseMessage,
      });
    },
  ...
  const extendStayMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest("POST", `/api/owner/bookings/${id}/extend`, {
        newCheckOutDate,
      });
    },
    onSuccess: (data: any) => {
  ...
  const noShowMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "The booking has been marked as no-show. The guest has been notified and the room inventory has been released.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleReject = () => {
    ...
    if (selectedRejectionReason === "other") {
      ...
      const selectedOption = REJECTION_REASONS.find(
        (r) => r.value === selectedRejectionReason,
      );
  ...
  const renderBookingTimeline = (status: string) => {
    const steps = [
      ...
      {
        label: "Awaiting Guest Confirmation",
    ...
    return (
      <div
        className="text-xs space-y-1 pt-3 border-t border-border/50"
  ...
  const getStatusBadge = (status: string) => {
    ...
    > = {
      ...
      confirmed: {
        variant: "default",
        label: "Accepted – Awaiting Guest Confirmation",
      },
      customer_confirmed: {
        variant: "default",
        label: "Guest Confirmed – Ready for Check-in",
      },
  ...
  const isUpcoming = (booking: Booking) => {
    ...
    return (
      checkIn > today ||
      (checkIn.getTime() === today.getTime() && !booking.checkInTime)
    );
  };
    ...
    <Card
      ...
      className={
        highlightBookingId === booking.id
          ? "ring-2 ring-orange-400 ring-offset-2 transition-all"
          : ""
      }
    >
          ...
          <div className="text-sm p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md flex items-center gap-2">
            ...
            <span className="text-amber-800 dark:text-amber-200">
              ...
              {booking.actualCheckOutDate &&
                format(new Date(booking.actualCheckOutDate), "dd MMM yyyy")}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link href="/owner/messages">
                <Button
                  size="sm"
                  variant="ghost"
                  data-testid={`message-guest-pending-${booking.id}`}
                >
            ...
            <div className="flex items-start gap-2 p-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                Accepting will temporarily hold this room for the guest. Payment
                is not collected yet.
              </p>
            </div>
            <p className="text-xs text-muted-foreground">
              Guest contact details will be available after you accept the
              booking.
            </p>
          ...
          <div className="space-y-3">
            <div className="p-2 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-md">
              <p className="text-xs text-green-700 dark:text-green-300">
                The guest has been notified. You'll be informed once the guest
                confirms or completes payment.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              {canCheckIn(booking) && (
              ...
              <Link href="/owner/messages">
                <Button
                  size="sm"
                  data-testid={`message-guest-confirmed-${booking.id}`}
                >
                  <MessageSquare className="h-4 w-4 mr-1" />
                  Message Guest
                </Button>
          ...
          <div className="space-y-3">
            <div className="p-2 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-md">
              <p className="text-xs text-green-700 dark:text-green-300 font-medium">
                The guest has confirmed their booking. Room is secured and ready
                for their arrival.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link href="/owner/messages">
                <Button
                  size="sm"
                  data-testid={`message-guest-customer-confirmed-${booking.id}`}
                >
                  <MessageSquare className="h-4 w-4 mr-1" />
                  Message Guest
                </Button>
          ...
          <div className="space-y-3">
            ...
            {booking.checkInTime && (
              <span className="text-xs text-muted-foreground">
                Checked in:{" "}
                {format(new Date(booking.checkInTime), "dd MMM HH:mm")}
              </span>
            )}
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap text-sm text-muted-foreground">
              ...
              {booking.checkOutTime && (
                <span>
                  Checked out:{" "}
                  {format(new Date(booking.checkOutTime), "dd MMM HH:mm")}
                </span>
              )}
            </div>
  ...
  return (
    <OwnerLayout>
      {showEscalationMsg && (
        <div
          className="mx-4 mt-3 p-3 bg-orange-50 dark:bg-orange-950/30 border border-orange-300 dark:border-orange-700 rounded-md flex items-center gap-2"
          data-testid="escalation-message"
        >
          <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400 shrink-0" />
          <p className="text-sm font-medium text-orange-700 dark:text-orange-300">
            Escalation process initiated. Please respond to this booking
            immediately.
          </p>
        </div>
      )}
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-lg space-y-2">
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  <p>
                    Scheduled Check-out:{" "}
                    {format(
                      new Date(earlyCheckoutBooking.checkOut),
                      "dd MMM yyyy",
                    )}
                  </p>
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2">
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  <p>
                    Current Check-out:{" "}
                    {format(
                      new Date(extendStayBooking.checkOut),
                      "dd MMM yyyy",
                    )}
                  </p>
                </div>
                  ...
                  <div className="text-sm text-muted-foreground mt-2">
                    {(() => {
                      const originalCheckout = new Date(
                        extendStayBooking.checkOut,
                      );
                      const nights = Math.ceil(
                        (extendDate.getTime() - originalCheckout.getTime()) /
                          (1000 * 60 * 60 * 24),
                      );
                      const price =
                        nights *
                        parseFloat(
                          extendStayBooking.property?.pricePerNight || "0",
                        );
                      return (
                        <>
          ...
          <div className="py-4 space-y-4">
            {bookingToMarkNoShow && (
              <div className="p-3 bg-muted rounded-md space-y-2 text-sm">
                <p>
                  <strong>Guest:</strong> {bookingToMarkNoShow.guest?.name}
                </p>
                <p>
                  <strong>Property:</strong>{" "}
                  {bookingToMarkNoShow.property?.title}
                </p>
                <p>
                  <strong>Check-in Date:</strong>{" "}
                  {format(new Date(bookingToMarkNoShow.checkIn), "dd MMM yyyy")}
                </p>
                <p>
                  <strong>Amount:</strong>{" "}
                  {formatCurrency(bookingToMarkNoShow.totalPrice)}
                </p>
              </div>
            )}
            <div className="space-y-2">
