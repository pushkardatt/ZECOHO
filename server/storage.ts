...
export class DatabaseStorage implements IStorage {
  ...
  async createSupportConversation(data: { userId: string; userRole: string; subject?: string }): Promise<SupportConversation> {
    const [conversation] = await db
  ...
  async getSupportConversation(id: string): Promise<SupportConversation | null> {
    const [conversation] = await db
      .select()
  ...
  async getSupportConversationsByUser(userId: string): Promise<SupportConversation[]> {
    return db
      .select()
      .from(supportConversations)
  ...
  async getActiveSupportConversation(userId: string): Promise<SupportConversation | null> {
    const [conversation] = await db
      .select()
      .from(supportConversations)
      .where(
  ...
  async updateSupportConversation(id: string, data: Partial<SupportConversation>): Promise<SupportConversation | null> {
    const [updated] = await db
  ...
  async escalateSupportConversation(id: string, reason: string): Promise<{ conversation: SupportConversation; ticket: SupportTicket } | null> {
  ...
  async closeSupportConversation(id: string): Promise<SupportConversation | null> {
  ...
  async markSupportMessagesAsRead(conversationId: string, senderType?: 'user' | 'ai' | 'admin'): Promise<void> {
  ...
  async getAllSupportConversations(filters?: { 
    ...
    assignedTo?: string;
    limit?: number;
  }): Promise<(SupportConversation & { user: User; unreadCount: number; lastMessage?: SupportMessage })[]> {
    const conditions: any[] = [];
    ...
    if (filters?.assignedTo) {
      conditions.push(eq(supportConversations.assignedAdminId, filters.assignedTo));
    }
  ...
  async assignSupportConversation(conversationId: string, adminId: string): Promise<SupportConversation | null> {
  ...
  // Support tickets
  async getSupportTickets(filters?: { status?: string; priority?: string }): Promise<(SupportTicket & { conversation: SupportConversation; user: User })[]> {
    const conditions: any[] = [];
    ...
    let query = db
      ...
      .from(supportTickets)
      .innerJoin(supportConversations, eq(supportTickets.conversationId, supportConversations.id))
      .innerJoin(users, eq(supportConversations.userId, users.id));
  ...
  async updateSupportTicket(id: string, data: Partial<SupportTicket>): Promise<SupportTicket | null> {
    const [updated] = await db
  ...
  async resolveSupportTicket(id: string, notes: string): Promise<SupportTicket | null> {
