...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  app.patch("/api/owner/bookings/:id/status", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const finalStatus = status === "confirmed" ? "customer_confirmed" : status;
      const updated = await storage.updateBookingStatus(req.params.id, finalStatus, responseMessage);
      ...
      if (guest?.email) {
        ...
        if (status === "confirmed") {
          ...
          sendBookingDeclinedEmail(...).catch(console.error);
        }
      }
